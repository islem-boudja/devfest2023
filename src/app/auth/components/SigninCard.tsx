  ...
  return (
    <div className={`sign-in-card rounded-xl w-1/2 h-full bg-violet-900 px-16`}>
        ...
        <form className='sign-in-form' onSubmit={(e) => handleSubmit(e)}>
            <input type="email" name="email" placeholder='Email' className='rounded-md p-2' onChange={(e) => setEmail(e.target.value)}/>
            <input type="password" name="password" placeholder='Password' className='rounded-md my-5 p-2' onChange={(e) => setPassword(e.target.value)}/>
            <div className='flex justify-start items-center w-72'>
                <input type="checkbox" name="check-sign-in" className='check-sign-in mr-2' />
                <label htmlFor="check-sign-in" className='text-white check-sign-in'>Keep me logged in</label>
            </div>
            <button className='sign-in-btn rounded-md text-white font-semibold w-72 mt-5 py-1 px-4'>Sign In</button>
        </form>
        <p>Need an account ? {' '}
            <button className='text-white font-semibold cursor-pointer' onClick={toggleCard}> Sign Up</button>
        </p>
